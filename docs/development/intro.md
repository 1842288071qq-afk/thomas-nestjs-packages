...
| **GlobalModule** | Module | 注册全局 Filters, Pipes, Interceptors (如 `DateSerializeInterceptor`)。 | `libs/core/src/nest` |
...
flowchart TD
    ...
    subgraph Interceptors & Pipes
        Interceptor["DateSerializeInterceptor"] --> Pipe["ValidationPipeWithTransform"]
    end
